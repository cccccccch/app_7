<template>
  <div class="cd-color-picker-wrap" ref="cdColorPickerWrap">
    <div class="cd-color-picker" ref="cdColorPicker" @touchstart.stop="onTouchStart" @touchmove.stop="onTouchMove" @touchend.stop="onTouchEnd">
      ...
      <div class="indicator" ref="indicator" :style="{ left: indicatorPosition.x + 'px', top: indicatorPosition.y + 'px', background: `rgb(${curColor[0] | 0},${curColor[1] | 0},${curColor[2] | 0})` }"></div>
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    _initIndicatorPosition(xyValue, disableEmit) {
      ...
      this.$nextTick(() => {
        ...
        !disableEmit && this._emitChange();
      });
    },
    _emitChange(isTouchEnd = false) {
      this.$emit("onColorChange", { color: this.curColor, curXY: this.curXY, isTouchEnd: isTouchEnd });
    },
  },
  created() {
    this._initColors();
