<template>
  <div class="device-options" :class="{ active: showAnimation }" ref="deviceOptions" @scroll="handleScroll">
    <transition name="opt-slide">
      <div class="options-func" v-show="showAnimation" v-if="childModeShow || deviceDetail.ifGroupAble || (deviceDetail.ifLogicGroupEnum === 'no' && deviceDetail.zigbeeTypeEnum === 'RGBCW2')">
        <p class="title">功能</p>
        <!-- 全彩灯和色温灯具有儿童模式 -->
        <div class="cell" @click="goChildMode" v-if="childModeShow">
          <div class="left">儿童模式</div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <div class="cell" @click="goGroup" v-if="deviceDetail.ifGroupAble">
          <div class="left">
            {{ deviceDetail.ifLogicGroupEnum === "no" ? "创建编组" : "编组管理" }}
          </div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <div class="cell" @click="goLightStatus">
          <div class="left">灯光状态</div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <div
          class="cell"
          @click="goLightSpeed"
          v-if="(deviceDetail && deviceDetail.ifLogicGroupEnum !== 'yes' && deviceDetail.typeCode !== 'DP04') || (isGroupSupportSlow && deviceDetail && deviceDetail.ifLogicGroupEnum === 'yes')"
        >
          <div class="left">缓亮缓灭设置</div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <div class="cell" @click="goBrightnessPage(0)" v-if="deviceDetail.deviceTypeCode === 'M006Dimm01'">
          <div class="left">调光曲线</div>
          <div class="right">
            <span class="tips name">{{ getBrightnessCurveValue === 1 ? "曲线调光" : "线性调光" }}</span>
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <div class="cell" @click="goBrightnessPage(1)" v-if="deviceDetail.deviceTypeCode === 'M006Dimm01'">
          <div class="left">调光方式</div>
          <div class="right">
            <span class="tips name">{{ getBrightnessWayValue === 1 ? "0-10V调光" : "1-10V调光" }}</span>
            <span class="iconfont icon-right"></span>
          </div>
        </div>
      </div>
    ...
    <transition name="opt-slide-two">
      <div class="options-normal" v-show="showAnimation">
        ...
        <div class="cell" @click="goFeedback" v-if="deviceDetail.ifLogicGroupEnum !== 'yes'">
    ...
    <transition name="opt-slide-three">
      <div class="options-info" v-show="showAnimation">
        <p class="title">信息</p>
        <div v-if="deviceDetail && deviceDetail.ifLogicGroupEnum && deviceDetail.ifLogicGroupEnum !== 'yes'">
          <div class="cell">
    ...
    <transition name="opt-slide-four">
      <div v-show="showAnimation">
        <div class="del-btn" v-if="deviceDetail.ifLogicGroupEnum !== 'yes'" @click="clickDel(0)">删除设备</div>
        <div class="del-btn" v-if="deviceDetail.ifLogicGroupEnum === 'yes'" @click="clickDel(1)">解散编组</div>
      </div>
    </transition>
    ...
    <Modal v-model="delModal" title="删除设备" content="设备删除后将从您的家庭中移除，确定要删除吗" @on-ok
    ="onDel" />
    <Modal v-model="delGroupModal" title="温馨提示" content="确认要解散灯组吗" @on-ok="onDel" />
  </div>
</template>
<script>
import iSwitch from "./switch";
import { deviceZigbeeType, throttle } from "../utils/index";
import { deviceInfoUpdRep, deviceDelRep, addUsuallyRep, removeUsuallyRep, groupDeviceListRep } from "../api/request";
...
export default {
  ...
  data() {
    return {
      ...
      isGroupSupportSlow: false,
      handleScroll: null
    };
  },
  methods: {
    ...mapMutations(["setDeviceDetail", "setDeviceDataModify", "setOptionPageScrollPosition"]),
    goLightSpeed() {
      this.$router.push({ name: "LightSpeed", query: { slideWay: "left" } });
    },
    onScroll(e) {
    ...
    goBrightnessPage(type) {
      this.$router.push({ name: "BrightnessOprate", query: { slideWay: "left", type: type } });
    },
    goScenePage() {
    ...
    async getGroupDeviceData() {
      let { data, error } = await groupDeviceListRep({
        dataId: this.deviceDetail.dataId,
        dataTypeEnum: this.deviceDetail.dataTypeEnum
      });
      this.isGroupSupportSlow = false;
      if (data && !error) {
        this.groupList = data.data || [];
        for (let item of this.groupList) {
          let t = item.typeCode && item.typeCode.toUpperCase();
          if (t.indexOf("M006") >= 0) {
            this.isGroupSupportSlow = true;
            break;
          }
        }
      }
 ...
 console.log('deviceDetail',this.deviceDetail)
  ...
  created() {
    ...
    if (this.deviceDetail.ifLogicGroupEnum === "yes") {
  ...
  computed: {
    ...mapGetters(["deviceDetail", "globalDataId", "globalDeviceStatus", "getBrightnessCurveValue", "getBrightnessWayValue", "getOptionPageScrollPosition"]),
    childModeShow() {
      return (
        this.deviceZigbeeType[this.deviceDetail.zigbeeTypeEnum] === "ZIGBEE_TYPE_HOME_ALL_LIGHT" || this.deviceZigbeeType[this.deviceDetail.zigbeeTypeEnum] === "ZIGBEE_TYPE_HOME_TEMPERATURE_LIGHT"
      );
    }
  },
  watch: {
