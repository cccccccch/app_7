<template>
  <div class="device-options" :class="{ active: showAnimation }">
    <transition name="opt-slide">
      ...
      <div class="options-normal" v-if="showAnimation && showFunctionGroup">
        <p class="title">功能</p>
    ...
    <transition name="opt-slide-three">
      <div class="options-info" v-if="showAnimation">
        ...
        <!-- <div class="cell" @click="goSignal" v-if="showSignalTest">
    ...
    <Modal v-model="deviceDisableModal" v-transclude="'#power-strip'"  title="温馨提示" type="one" :content="deviceDisableContent" />
    <Modal v-model="delModal" v-transclude="'#power-strip'" title="删除设备" content="设备删除后将从您的家庭中移除，确定要删除吗" @on-ok="onDel" />
    <!-- 设备查找 ing Modal -->
    <Modal
      v-transclude="'#power-strip'"
      v-model="findDeviceModal"
      title="查找设备"
      type="one"
      content="设备已唤醒，请注意查看"
      @on-ok="stopFindDevice"
    >
    ...
    <!-- 设备查找 超时Modal -->
     <Modal 
      v-transclude="'#power-strip'"
      v-model="findDeviceTimeoutModal"
      title="温馨提示" 
      content="查找超时，请点击重试"
      okText="重试"
      @on-ok="handleFindDevice"
    >  
    </Modal>
  </div>
</template>
...
import { transclude } from '@/directive'
export default {
  name: "DeviceOptions",
  mixins: [onoffswitch],
