  ...
  <div class="device-options" :class="{ active: showAnimation }">
    <transition name="opt-slide">
      <div class="options-func" v-show="showAnimation">
        <p class="title">功能</p>
        <div class="cell" @click="goCorrectPage" v-if="curtainType !== 9">
          <div class="left">上下限位设置</div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <div class="cell" @click="goPreference">
          <div class="left">偏好设置</div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        ...
        <!-- <div class="cell" @click="goGearPage" v-if="curtainType === 3">
          <div class="left">翻转档位设置</div>
          <div class="right">
            <span class="tips name">{{ (getCurtainAngleGear && getCurtainAngleGear + "档") || "" }}</span>
            <span class="iconfont icon-right"></span>
          </div>
        </div> -->
    ...
    <transition name="opt-slide-two">
      <div class="options-normal" v-show="showAnimation">
        ...
        <div class="cell" @click="clickCurtainType" v-if="curtainType !== 9">
          <div class="left">窗帘类型重置</div>
          <div class="right">
            <span class="tips name">{{ deviceTypeName }}</span>
            <span class="iconfont icon-right"></span>
          </div>
        </div>
...
import { deviceInfoUpdRep, deviceDelRep, addUsuallyRep, removeUsuallyRep, endpointControlRep } from "../../../api/request";
...
export default {
  ...
  methods: {
    ...mapMutations(["setDeviceDetail", "setDeviceDataModify", "setCurtainRunWay"]),
    // 非在线状态弹窗
    _showOfflineModal() {
  ...
  computed: {
    ...mapGetters(["deviceDetail", "globalDataId", "globalDeviceStatus", "getCurtainAngleGear", "getCurtainTypeCodes", "getDoorAutoLockTime", "getCurtainRunWay", "globalAppVersion"]),
    childModeShow() {
