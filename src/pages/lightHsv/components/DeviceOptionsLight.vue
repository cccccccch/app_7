<template>
  <div class="device-options" :class="{ active: showAnimation }" @scroll="handleScroll" ref="deviceOptions">
    <transition name="opt-slide">
      <div class="options-func" v-show="showAnimation" v-if="childModeShow || deviceDetail.ifGroupAble">
        <p class="title">功能</p>
        <!-- 全彩灯和色温灯具有儿童模式 -->
        ...
        <div class="cell" @click="goLightStatus" v-if="deviceDetail.zigbeeTypeEnum !== 'CW_BW' && deviceDetail.zigbeeTypeEnum !== 'RGBCW_BW'">
          <div class="left">灯光状态</div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <div class="cell" @click="goLightSpeed" v-if="deviceDetail.zigbeeTypeEnum !== 'CW_BW' && deviceDetail.zigbeeTypeEnum !== 'RGBCW_BW' ">
          <!-- && deviceDetail.zigbeeTypeEnum !== 'RGBCW3' -->
    ...
    <transition name="opt-slide-three">
      <div class="options-info" v-show="showAnimation">
        ...
        <div v-if="deviceDetail && deviceDetail.ifLogicGroupEnum && deviceDetail.ifLogicGroupEnum !== 'yes'">
          ...
          <!-- <div class="cell" @click="goSignal" v-if="deviceDetail.zigbeeTypeEnum !== 'CW_BW' && deviceDetail.zigbeeTypeEnum !== 'RGBCW_BW'">
          ...
          <div class="cell" @click="goVersion">
            ...
            <div class="right">
              ...
              <span class="iconfont icon-right" v-show="deviceDetail.zigbeeTypeEnum !== 'RGBCW_BW' && deviceDetail.zigbeeTypeEnum !== 'CW_BW'"></span>
            </div>
          </div>
        </div>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    goVersion() {
      if (this.deviceDetail.zigbeeTypeEnum === "RGBCW_BW" || this.deviceDetail.zigbeeTypeEnum === "CW_BW") return;
      if (!this._showOfflineModal()) return;
      if (!this.deviceDetail) {
        this.$Toast.show({ title: "参数错误" });
        return;
  ...
  computed: {
    ...mapGetters(["deviceDetail", "globalDataId", "globalDeviceStatus", "getOptionPageScrollPosition"]),
    childModeShow() {
      return (
        (this.deviceDetail.zigbeeTypeEnum === "RGBCW3" || this.deviceDetail.zigbeeTypeEnum === "CW3") && this.deviceDetail.zigbeeTypeEnum !== "CW_BW" && this.deviceDetail.zigbeeTypeEnum !== "RGBCW_BW"
      );
    }
  },
  watch: {
