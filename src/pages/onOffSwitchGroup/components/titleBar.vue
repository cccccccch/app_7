<template>
  <div class="title-bar">
    ...
    <transition name="slide">
      <DeviceOptionsOnOffSwitchGroup v-if="active" :isShow="active" />
    </transition>
  </div>
</template>
<script>
import DeviceOptionsOnOffSwitchGroup from "./DeviceOptions";
export default {
  name: "OnOffSwitchtitleBar",
  props: {
    title: String,
  ...
  methods: {
    clickOption() {
      ...
      this.$emit('handleScheduleTaskResult',1)
    },
    goback() {
      this.$emit("goBack");
    },
