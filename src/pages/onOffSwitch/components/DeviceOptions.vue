<template>
  <div class="device-options" ref="options" :class="{ active: showAnimation }" @scroll="handleScroll">
    <transition name="opt-slide">
      ...
      <div class="options-normal" v-if="showAnimation && (showFunctionGroup || showCreateGroup)">
        <p class="title">功能</p>
        <!-- showCreateGroup from mixin -->
        <div class="cell" @click="goEditGroup" v-if="showCreateGroup">
          <div class="left">创建编组</div>
        ...
        <div class="cell" @click="goCountdownSetting" v-if="showCountdown" :class="onOffSwithDeviceStatus === 5 ? 'disabled' : ''">
        ...
        <!-- showIntelligence from mixin -->
        <div class="cell" @click="goBindSetting" v-if="showIntelligence">
          <div class="left">关联智能</div>
          <div class="right">
            <span class="tips">{{ onOffSwithDeviceStatus === 0 ? "关闭" : "开启" }}</span>
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <!-- showLightDoNotDisturb from mixin -->
        <div class="cell" v-if="showLightDoNotDisturb" @click="setDoNotDisturbMode">
          <div class="left">勿扰模式</div>
          <div class="right">
            <span class="tips">{{ doNotDisturbEnableText }}</span>
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        ...
        <!-- showFindDevice from mixin -->
        <div class="cell" @click="handleFindDevice" v-if="showFindDevice">
          <div class="left">设备查找</div>
          <div class="right">
    ...
    <transition name="opt-slide-three">
      <div class="options-info" v-if="showAnimation">
        ...
        <!-- 存在UDID才显示 -->
        <div class="cell" v-if="showUDID">
          <div class="left">UDID</div>
          <div class="right">
            <span class="tips">{{ deviceDetail.bindUdid || "-" }}</span>
          </div>
        </div>
        <!-- <div class="cell" @click="signalTest" v-if="showSignalTest">
    ...
    <Modal
      ...
      okText="我知道了"
    />
    <Modal v-transclude="'#onOffSwitch'" v-model="delModal" title="删除设备" content="设备删除后将从您的家庭中移除，确定要删除吗" @on-ok="onDel" />
    <Modal v-transclude="'#onOffSwitch'" v-model="statusData.show" title="温馨提示" type="one" :content="statusData.content" okText="确认" />
    <!-- 设备查找 ing Modal -->
    <Modal v-transclude="'#onOffSwitch'" v-model="findDeviceModal" title="查找设备" type="one" content="设备已唤醒，请注意查看" @on-ok="stopFindDevice">
    ...
    <!-- 设备查找 超时Modal -->
    <Modal v-transclude="'#onOffSwitch'" v-model="findDeviceTimeoutModal" title="温馨提示" content="查找超时，请点击重试" okText="重试" @on-ok="handleFindDevice"> </Modal>
  </div>
</template>
<script>
import iSwitch from "../../../components/switch";
import { deviceInfoUpdRep, deviceDelRep, removeUsuallyRep, addUsuallyRep, endpointControlRep } from "../../../api/request";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters({ getTimingDesc: "schedule/getTimingDesc" }),
    ...
    showRelaySetting() {
      ...
      const { function_power_on_relay_setting } = attrs;
      return function_power_on_relay_setting == 1;
    },
    relayStatusDisabled() {
