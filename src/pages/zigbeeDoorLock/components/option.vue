<template>
  <div class="device-options" :class="{ active: showAnimation, hidden: showPicker }" @scroll="handleScroll" ref="deviceOptions">
    <transition name="opt-slide">
      <div class="options-func" v-show="showAnimation">
        <p class="title">功能</p>
        <div class="cell" @click="goAuth">
        ...
        <!-- <div class="cell">
          <div class="left">胁迫密码</div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div>
        <div class="cell" @click="clickAutoLock">
          <div class="left">自动上锁时间</div>
          <div class="right">
            <span class="iconfont icon-right"></span>
          </div>
        </div> -->
    ...
    <transition name="opt-slide-four">
      <div v-show="showAnimation">
        <div class="del-btn" @click="clickDel(0)">删除设备</div>
      </div>
    </transition>
    ...
    <div class="picker-outer" @touchstart.stop @touchmove.stop :class="{ active: showPicker }">
      <div class="mask" v-show="showPicker"></div>
      <picker style="position: relative;z-index: 2" pickerType="second" :value="getDoorAutoLockTime" @onConfirm="onPickerConfirm" @onCancel="onPickerCancel" />
    </div>
  </div>
</template>
<script>
...
import { deviceInfoUpdRep, deviceDelRep, addUsuallyRep, removeUsuallyRep, doorControlReq } from "../../../api/request";
...
export default {
  ...
  methods: {
    ...
    clickAutoLock() {
      if (this.globalDeviceStatus !== 1 && this.globalDeviceStatus !== 5) {
    ...
    async handleSetSecond(data) {
      let result = await doorControlReq({ cmdKey: "autoCloseLockSeconds", cmdValue1: data.second, endpointId: this.deviceDetail.dataId });
      let msg = result && !result.error ? "设置成功" : "设置失败";
  ...
  computed: {
    ...mapGetters(["deviceDetail", "globalDataId", "globalDeviceStatus", "getDoorAutoLockTime", "getOptionPageScrollPosition"]),
    childModeShow() {
